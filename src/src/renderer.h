		...
		if (random_double() > alpha) { // 穿透
			...
			for (shared_ptr<light> light_ptr : light_ptr_list)
			{
				...
				else
				{
					...
					double pdf_p;
					vec3 normali, positioni;
					tie(pdf_p, normali, positioni) = rec.mat_ptr->sample_positioni(normalo, positiono, bvh_root); // 获取入射点采样pdf，入射点法线，入射方向
					vec3 brdf_light = rec.mat_ptr->brdf(wo, normalo, positiono, rec.uv, wi_light, normali, positioni); // 计算到光源的brdf
			...
			if (scatter)
			{
				...
				vec3 brdf = rec.mat_ptr->brdf(wo, normalo, positiono, rec.uv, wi, normali, positioni);
				ray new_ray(positioni + wi * 0.0001, wi); // 新的光线从入射点射出
				vec3 radiance_indirect = ray_color(new_ray, bvh_root, depth - 1, light_ptr_list) * brdf * dot(normalo, wi) / pdf_w / P_RR;
